...
Atom::Atom (const std::string& name, const coord_t& position, const coord_t& force) :
  _name(name), _residue(""), _ID(-1), _molid(-1),
  _pmolecule((Molecule *)NULL),
  _mass(0.0), _charge(0.0),
  _position(position), _force(force), _element(NO_ELEMENT)
{ this->SetAtomProperties (); }

Atom::Atom (const coord_t& position, const coord_t& force) :
  _name(""),
  _residue(""),
  _ID(-1),
  _molid(-1),
...
Atom::Element_t Atom::String2Element (const std::string& name) {
	VecR p, f;
	AtomPtr atom = new Atom(name, p, f);
