	...
	*/
	...
	template <typename T>
		class CycleManipulator : public SystemManipulator<T> {
			public:
				...
				std::list<cycle_list>::const_iterator	cycle_begin() const { return cycle.begin(); }
				std::list<cycle_list>::const_iterator	cycle_end() const { return cycle.end(); }

				std::list<cycle_t>::const_iterator		cycle_type_begin() const { return cycle_type.begin(); }
				std::list<cycle_t>::const_iterator		cycle_type_end() const { return cycle_type.end(); }
	...
	template <typename T>
		void CycleManipulator<T>::ParseCycles () { 
			...
			std::list<AtomPtr>::const_iterator source, target;
			gray_source = vis.gray_source_begin();
			gray_target = vis.gray_target_begin();

			cycle.clear();
			cycle_type.clear();

			while (gray_source != vis.gray_source_end() && gray_target != vis.gray_target_end()) {
	...
	template <typename T>
	  void CycleManipulator<T>::FindUniqueMembers (cycle_list& _cycle) {
	...
	template <typename T>
	  CycleManipulator<T>::cycle_pair_t CycleManipulator<T>::CyclePointAtom (cycle_list& _cycle) {
	...
	template <typename T>
	  std::pair<int,int> CycleManipulator<T>::WaterLegInformation (cycle_list& _cycle) {
	...
	template <typename T>
	  void SO2BondingCycleAnalysis<T>::Analysis() {
		...
		std::list<CycleManipulator<T>::cycle_list>::const_iterator	cycle = cm.cycle_begin();	
		std::list<CycleManipulator<T>::cycle_t>::const_iterator		cycle_type = cm.cycle_type_begin();
